...
public class SocketManager
{
    ...
    private void _onConnet()
    {
        try
        {
            ...
            IAsyncResult result = clientSocket.BeginConnect(ipEndpoint, new AsyncCallback(_onConnect_Sucess), clientSocket);//异步连接（异步操作完成时就把clientSocket对象当做参数传给委托方法）
            bool success = result.AsyncWaitHandle.WaitOne(5000, true);//(阻止当前线程，直到当前的 WaitHandle 收到信号为止，同时使用 32 位带符号整数指定时间间隔，并指定是否在等待之前退出同步域)
            if (!success) //超时
            {
                _onConnect_Outtime();
            }
    ...
    private void _onConnect_Sucess(IAsyncResult iar)
    {
        try
        {
            Socket client = (Socket)iar.AsyncState;//（这个就是异步传过来的参数）
            client.EndConnect(iar);//(结束挂起的异步连接请求)
    ...
    private void _onReceiveSocket()
    {
        while (true)
        {
            ...
            try
            {
                ...
                if (receiveLength > 0)
                {
                    ...
                    while (_databuffer.GetData(ConstantsTAG.DataHead_Str,ConstantsTAG.DataEnd_Str, out _socketData))
                    {
                        ...
                        {
                            //Debug.Log(tmpNetMessageData._eventType);//输出网络信息数据的传输类型
            ...
            catch (System.Exception e)
            {
                ...
                clientSocket.Shutdown(SocketShutdown.Both);//禁用发送和接收的 Socket
                clientSocket.Close();
                break;
            }
        }
